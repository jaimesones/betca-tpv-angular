...
export class ShoppingCartService {
  ...
  constructor(private dialog: MatDialog, private articleService: SharedArticleService,
              private invoiceService: SharedInvoiceService, private httpService: HttpService) {
  }
  ...
  createInvoiceAndPrint(ticketId: string): Observable<void> {
    return this.invoiceService.create(ticketId)
      .pipe(
        concatMap(invoice => {
          return this.httpService.pdf().get(EndPoints.INVOICES + '/' + invoice.identity + ShoppingCartService.RECEIPT);
        })
      );
  }
